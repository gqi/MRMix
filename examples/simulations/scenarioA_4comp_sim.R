#' This is the code to generate the Scenario A simulation results of the MRMix paper.
#' Summary statistics are simulated using the 4-component model.
#' Results are presented in Figures 2 and 3, Supplementary Figures 1-3, Supplementary Table 5.
#' Usage: Rscript scenarioA_4comp_sim.R arg1 arg2 arg3 arg4 arg5
#' arg1-5 are command line arguments to be passed to the script
#' arg1 indexes Nvec and can take values 1, 2, 3, 4, 5. For example, arg1=2 means N=1e5 and arg1=3 means N=2e5.
#' arg2 indexes pi1vec and can take values 1, 2. arg2=1 means pi1=0.005 and arg2=2 means pi1=0.01.
#' arg3 indexes pthrvec and can take values 1, 2, 3, 4. For example, arg3=1 means pthr=0.005 and arg3=4 means pthr=5e-8.
#' arg4 indexes thetavec and can take values 1, 2, 3. For example, arg4=3 means theta=0.2.
#' arg5 indexes NxNy_ratio_vec and can take values 1, 2, 3, 4, 5. For example, arg5=4 means NxNy_ratio=4.
#'
#' Examples of usage:
#' N=1e5, 50% valid IVs (pi1=0.01), instrument selection threshold p<5e-8, causal effect 0.2, ny=nx/3
#' Rscript scenarioA_4comp_sim.R 2 2 4 3 3
...
NxNy_ratio = NxNy_ratio_vec[temp[5]] # Ratio of sample size for X and Y
...
nx.rev = ny; ny.rev = nx # Sample size reverse directional analysis
...
for (repind in 1:100){
    ...
    if (est[repind,6]>2) {
        ...
        theta_est = MRMix(betahat.x, betahat.y, 1/nx, 1/ny, theta_temp_vec, pi_init = 0.6, sigma_init = 1e-5)
    }
    ...
    if (est[repind,12]>2) {
        ...
        theta_temp_vec = seq(-0.49,0.5,by=0.01)
        theta_est = MRMix(betahat.x.rev, betahat.y.rev, 1/nx.rev, 1/ny.rev, theta_temp_vec, pi_init = 0.6, sigma_init = 1e-5)
