...
#' Rscript scenarioC_4comp_laplace.R 2 2 4 2 2
#'
...
for (repind in 1:100){
    ...
    beta[ind1,1] = rlaplace(length(ind1), location=0, scale=sqrt(sigma2x)) # The scale paramter is the SD
    beta[ind1,2] = theta*beta[ind1,1]
    beta[ind2,] = rmLaplace(length(ind2), center = c(0,0), Scatter = matrix(c(sigma2x,rho*sqrt(sigma2x*sigma2y),rho*sqrt(sigma2x*sigma2y),sigma2y),nrow=2))
    beta[ind2,2] = beta[ind2,2] + theta*beta[ind2,1]
    beta[ind3,2] = rlaplace(length(ind3), location = 0, scale = sqrt(sigma2y))
    ...
    if (est[repind,6]>2) {
        ...
        theta_est = MRMix(betahat.x, betahat.y, sx2=1/nx, sy2=1/ny, theta_temp_vec, pi_init = 0.6, sigma_init = 1e-5)$theta
    }
    ...
    if (est[repind,12]>2) {
        ...
        theta_temp_vec = seq(-0.49,0.5,by=0.01)
        theta_est = MRMix(betahat.x.rev, betahat.y.rev, sx2=1/nx.rev, sy2=1/ny.rev, theta_temp_vec, pi_init = 0.6, sigma_init = 1e-5)$theta
